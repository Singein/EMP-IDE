  ...
  export default {
    ...
    methods: {
      ...
      handleContentMenuClick(node) {
        ...
        else if (code === 'deleteFile')
          command = emp.delFile(this.menuTarget);
      ...
      renderContent(h, {node,data,store}) {
        if (data.children) {
          if (node.label === "/") {
            return ( <mu-flex align-items="center" class="tree-node" >
              <mu-icon value = "folder" size = "22"/>
              <span class="tree-node-label">{node.label}</span></mu-flex>
            );
          }
          return ( <mu-flex align-items="center" class="tree-node" >
            <mu-icon value="folder" size="22"/>
            <span class = "tree-node-label">{node.label.split("/").slice(-1)}</span>
            </mu-flex>
          );
        } else {
          return ( <mu-flex align-items="center" >
