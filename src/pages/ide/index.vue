<template>
  <div>
                ...
                <div class="inner-container">
                  ...
                  <mu-list toggle-nested class="ide-list content" :value="list_index" @change="handleChange">
                    ...
                    <mu-list-item v-for="(i,index) in root_files.children" :key="index" :value="i.index"
                      button :ripple="false" @click="get_code('',i.name,i.children)" nested>
                      ...
                      <!-- </mu-tooltip> -->
                      <!-- 二级目录 -->
                      <mu-list-item v-for="j in i.children" :key="j.name" button :ripple="false"
                        @click="get_code(i.name,j.name,false)" slot="nested" :value="j.index">
            ...
            <mu-expansion-panel class="ide-panel" :expand="panel === 'panel3'" color="grey" @change="toggle('panel3')">
              ...
              <mu-flex justify-content="end" align-items="center" direction="column" class="ide-panel-flex">
                ...
                <mu-flex style="width:100%;padding-top:6px" align-items="center" justify-content="end">
                  <mu-button small :disabled="!is_connected" @click="file_input()" color="indigo400"
                    style="margin:0 6px">
                    Browser...
                  </mu-button>
    ...
    <!-- 设置窗口 -->
    <mu-dialog title="MicroIDE Settings" width="400" max-width="80%" :esc-press-close="false"
      :overlay-close="false" :open.sync="openSetting">

      <mu-flex direction='column'>
        <mu-text-field label="Url" :disabled="is_connected" color="primary" v-model="url"
          full-width placeholder="ws://192.168.xxx.xxx:8266/"></mu-text-field>

        <mu-text-field label="Password" :disabled="is_connected" color="primary" v-model="passwd"
          full-width placeholder="password" type="password"></mu-text-field>

        <mu-text-field label="Editor fontsize" color="primary" v-model="fontSize" full-width
      ...
      new_folder() {
        if (this.is_connected) {
          if (this.new_folder_name.length > 0) {
            this.last_command = "create_folder('" + this.new_folder_name +
              "')\r";
            this.ws.send(this.last_command);
            this.panel = "";
          } else {
              ...
              switch (this.binary_state) {
                case 11:
                  ...
                  if (this.decode_resp(data) == 0) {
                    ...
                    for (
                      var offset = 0; offset < put_file_data.length; offset +=
                      1024
                    ) {
                      this.ws.send(put_file_data.slice(offset, offset +
                        1024));
                    }
